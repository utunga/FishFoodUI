...
namespace FishFood.Quadtree
{
    ...
    public class QuadTree<T>
    {
        ...
        public IEnumerable<QuadTreePositionItem<T>> NearByItems(QuadTreePositionItem<T> target, int countNeeded)
        {
            var nearEnoughToConsider = new List<QuadTreePositionItem<T>>();
            QuadTreeNode<T> containingNode =
                (target.LastAddedToQuadTreeNode != null &&
                 target.LastAddedToQuadTreeNode.ParentNode != null)
                    ? target.LastAddedToQuadTreeNode.ParentNode
                    : headNode;
            containingNode.GetAllItems(ref nearEnoughToConsider);
            while (nearEnoughToConsider.Count-1 < countNeeded && containingNode != headNode)
            {
                // not enough at this level of quadtree so..
                // go up one level..
                containingNode = containingNode.ParentNode;
                // try again..
                nearEnoughToConsider = new List<QuadTreePositionItem<T>>();
                containingNode.GetAllItems(ref nearEnoughToConsider);
            }
            //return nearEnoughToConsider.Where(near => near != target).Take(countNeeded);
            return nearEnoughToConsider.Where(near => near != target).OrderBy(
                   near => (target.Position - near.Position).Length()).Take(countNeeded);
        }
    }
}
